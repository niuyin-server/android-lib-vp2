...
object DataUtil {
    ...
    @JvmStatic
    fun productImageFrontDatas(
        mAdapter: SmartViewPager2Adapter<SourceBean>,
        produceSize: Int = 10
    ): MutableList<SourceBean> {
    ...
    @JvmStatic
    fun productImageBackDatas(
        mAdapter: SmartViewPager2Adapter<SourceBean>,
        produceSize: Int = 10
    ): MutableList<SourceBean> {
        return productDatas(
            (mAdapter.getItem(mAdapter.itemCount - 1) as SourceBean).id + 1,
            true,
            true,
            produceSize
        )
